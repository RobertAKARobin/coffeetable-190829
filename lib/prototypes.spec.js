o.spec('@array', ()=>{
	...
	o('.extractFrom()', ()=>{
		o(['foo'].extractFrom({foo: 'bar'})).deepEquals({foo: 'bar'})
		o(['foo', 'bar'].extractFrom({foo: 'bar'})).deepEquals({foo: 'bar'})
		o(['foo'].extractFrom({foo: 'bar', fizz: 'buzz'})).deepEquals({foo: 'bar'})
		o(['foo'].extractFrom()).equals(undefined)
		o([1].extractFrom('banana')).deepEquals({1: 'a'})
		o(['length'].extractFrom([1,2,3])).deepEquals({length: 3})
	})
	o('.first()', ()=>{
		const obj = {foo: 'bar'}
		const a = [obj, 'b', 'c']
